...
import { Button, Card, Hyperlink } from "@aegov/design-system-react";
...
export function ParticipantSidebar({ active, onNavigate }: ParticipantSidebarProps) {
  ...
  return (
    <aside className="relative hidden w-64 flex-shrink-0 flex-col overflow-hidden bg-brand-navy p-5 md:flex">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Button
              ...
              block
              variant="link"
              className={`h-auto justify-start gap-3 rounded-xl border-transparent px-4 py-3 text-start text-[15px] ${
      ...
      <Card variant="news" className="relative mt-7 rounded-2xl border border-white/[0.16] bg-white/10 p-4 text-white backdrop-blur-sm">
      ...
      <div className="relative mt-auto flex flex-col gap-3.5">
        <Hyperlink asChild className="px-2.5 text-[13px] !text-white/65 !no-underline hover:!text-white">
          <Link href="/parent">{t("parentLink")}</Link>
        </Hyperlink>
